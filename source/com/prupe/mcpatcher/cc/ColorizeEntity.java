...
public class ColorizeEntity {
   ...
   private static final Integer[] dyeColors = new Integer[colorNames.length];
   private static final float[][] fleeceColors = new float[colorNames.length][];
   private static final float[][] collarColors = new float[colorNames.length][];
   private static final float[][] armorColors = new float[colorNames.length][];
   ...
   public static int colorizeXPOrb(int origColor, float timer) {
      return xpOrbColors != null && xpOrbColors.length != 0 ? xpOrbColors[(int)((Math.sin(timer / 4.0) + 1.0) * (xpOrbColors.length - 1) / 2.0)] : origColor;
   }

   public static int colorizeXPOrb(int origRed, int origBlue, float timer) {
      int color = colorizeXPOrb(origRed << 16 | 0xFF | origBlue, timer);
