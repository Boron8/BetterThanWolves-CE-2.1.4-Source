...
public class Wrapper15 {
   ...
   public static void setupTexture(
      RenderEngine renderEngine, BufferedImage image, int glTextureId, boolean blur, boolean clamp, FakeResourceLocation textureName
   ) {
      if (image != null) {
   ...
   public static void copySubTexture(Texture dst, Texture src, int x, int y, boolean flipped) {
      if (flipped && !flippedTextureLogged) {
         ...
         logger.warning("copySubTexture(%s, %s, %d, %d, %s): flipped texture not yet supported", dst.getTextureName(), src.getTextureName(), x, y, flipped);
      }
   ...
   private static void copySubTexture(IntBuffer[] mipmaps, int x, int y, int width, int height) {
      if (mipmaps != null) {
         for (int level = 0; level < mipmaps.length; level++) {
            ...
            if (mipmap != null) {
               GL11.glTexSubImage2D(3553, level, x, y, width, height, 6408, 5121, mipmap);
            }
   ...
   public static BufferedImage addAABorder(String name, BufferedImage input) {
      return input != null && currentAtlas != null && MipmapHelper.useMipmapsForTexture(currentAtlas.basePath)
         ? AAHelper.addBorder(new FakeResourceLocation(currentAtlas.basePath), input)
         : input;
   }

   public static TextureStitched createSprite(String name) {
      return currentAtlas == null ? new TextureStitched(name) : BorderedTexture.create(currentAtlas.basePath, name);
   }
}
