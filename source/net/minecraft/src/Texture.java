...
public class Texture {
   ...
   private Texture(String par1Str, int par2, int par3, int par4, int par5, int par6, int par7, int par8, int par9) {
      ...
      if (par2 != 2) {
         ...
         GL11.glTexParameteri(this.textureTarget, 10242, par6);
         GL11.glTexParameteri(this.textureTarget, 10243, par6);
      } else {
         this.glTextureId = -1;
      }
   ...
   public void func_104062_b(int par1, int par2, Texture par3Texture) {
      if (this.textureNotModified) {
         ...
         GL11.glTexSubImage2D(
            ...
            par1,
            par2,
            par3Texture.getWidth(),
            par3Texture.getHeight(),
            this.textureFormat,
            5121,
            (ByteBuffer)((Buffer)par3Texture.getTextureData()).position(0)
         );
         ...
         if (var2 <= this.width && var3 <= this.height) {
            ...
            for (int var10 = 0; var10 < this.height; var10++) {
               for (int var11 = 0; var11 < this.width; var11++) {
                  ...
                  int var13 = var12 * 4;
                  var9[var13 + var6[0]] = (byte)(var7[var12] >> 24 & 0xFF);
                  var9[var13 + var6[1]] = (byte)(var7[var12] >> 16 & 0xFF);
                  var9[var13 + var6[2]] = (byte)(var7[var12] >> 8 & 0xFF);
                  var9[var13 + var6[3]] = (byte)(var7[var12] >> 0 & 0xFF);
               }
            }
            ...
            ((Buffer)this.textureData).limit(var9.length);
            if (this.autoCreate) {
               this.uploadTexture();
            } else {
               this.textureNotModified = false;
   ...
   public void uploadByteBufferToGPU(int iDestX, int iDestY, ByteBuffer buffer, int iBufferWidth, int iBufferHeight) {
      GL11.glBindTexture(this.textureTarget, this.glTextureId);
      GL11.glTexSubImage2D(
         this.textureTarget, 0, iDestX, iDestY, iBufferWidth, iBufferHeight, this.textureFormat, 5121, (ByteBuffer)((Buffer)buffer).position(0)
      );
      this.textureNotModified = true;
   }
}
