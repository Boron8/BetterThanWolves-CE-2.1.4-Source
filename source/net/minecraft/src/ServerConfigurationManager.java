...
public abstract class ServerConfigurationManager {
   ...
   public void initializeConnectionToPlayer(INetworkManager par1INetworkManager, EntityPlayerMP par2EntityPlayerMP) {
      ...
      par2EntityPlayerMP.a(this.mcServer.worldServerForDimension(par2EntityPlayerMP.dimension));
      ...
      var7.sendPacketToPlayer(
         new Packet1Login(
            par2EntityPlayerMP.entityId,
            var5.M().getTerrainType(),
            par2EntityPlayerMP.theItemInWorldManager.getGameType(),
            var5.M().isHardcoreModeEnabled(),
            var5.provider.dimensionId,
            var5.difficultySetting,
            var5.Q(),
            this.getMaxPlayers()
         )
      );
      ...
      this.func_96456_a((ServerScoreboard)var5.W(), par2EntityPlayerMP);
      this.updateTimeAndWeatherForPlayer(par2EntityPlayerMP, var5);
      this.sendPacketToAllPlayers(new Packet3Chat(EnumChatFormatting.YELLOW + par2EntityPlayerMP.ax() + EnumChatFormatting.YELLOW + " joined the game."));
      this.playerLoggedIn(par2EntityPlayerMP);
      var7.setPlayerLocation(
         par2EntityPlayerMP.posX, par2EntityPlayerMP.posY, par2EntityPlayerMP.posZ, par2EntityPlayerMP.rotationYaw, par2EntityPlayerMP.rotationPitch
      );
      this.mcServer.getNetworkThread().addPlayer(var7);
      var7.sendPacketToPlayer(new Packet4UpdateTime(var5.H(), var5.I()));
      ...
      for (PotionEffect var9 : par2EntityPlayerMP.bC()) {
   ...
   protected void func_96456_a(ServerScoreboard par1ServerScoreboard, EntityPlayerMP par2EntityPlayerMP) {
      ...
      for (ScorePlayerTeam var5 : par1ServerScoreboard.g()) {
         par2EntityPlayerMP.playerNetServerHandler.sendPacketToPlayer(new Packet209SetPlayerTeam(var5, 0));
      }

      for (int var9 = 0; var9 < 3; var9++) {
         ScoreObjective var10 = par1ServerScoreboard.a(var9);
         if (var10 != null && !var3.contains(var10)) {
            for (Packet var8 : par1ServerScoreboard.func_96550_d(var10)) {
   ...
   public void setPlayerManager(WorldServer[] par1ArrayOfWorldServer) {
      this.playerNBTManagerObj = par1ArrayOfWorldServer[0].L().getSaveHandler();
   }
   ...
   public int getEntityViewDistance() {
      return ChunkTracker.getFurthestViewableBlock(this.getViewDistance());
   }

   public NBTTagCompound readPlayerDataFromFile(EntityPlayerMP par1EntityPlayerMP) {
      NBTTagCompound var2 = this.mcServer.worldServers[0].M().getPlayerNBTTagCompound();
      NBTTagCompound var3;
      if (par1EntityPlayerMP.c_().equals(this.mcServer.getServerOwner()) && var2 != null) {
         par1EntityPlayerMP.f(var2);
         var3 = var2;
   ...
   public void playerLoggedOut(EntityPlayerMP par1EntityPlayerMP) {
      ...
      if (par1EntityPlayerMP.ridingEntity != null) {
         var2.e(par1EntityPlayerMP.ridingEntity);
         System.out.println("removing player mount");
      }

      var2.e(par1EntityPlayerMP);
   ...
   public EntityPlayerMP createPlayerForUser(String par1Str) {
      ...
      return (EntityPlayerMP)EntityList.createEntityOfType(
         EntityPlayerMP.class, this.mcServer, this.mcServer.worldServerForDimension(0), par1Str, (ItemInWorldManager)var6
      );
   }
   ...
   public void transferPlayerToDimension(EntityPlayerMP par1EntityPlayerMP, int par2) {
      ...
      for (PotionEffect var7 : par1EntityPlayerMP.bC()) {
      ...
      par1EntityPlayerMP.timeOfLastDimensionSwitch = var5.I();
   }
   ...
   public boolean areCommandsAllowed(String par1Str) {
      return this.ops.contains(par1Str.trim().toLowerCase())
         || this.mcServer.isSinglePlayer()
            && this.mcServer.worldServers[0].M().areCommandsAllowed()
   ...
   public List findPlayers(
      ChunkCoordinates par1ChunkCoordinates, int par2, int par3, int par4, int par5, int par6, int par7, Map par8Map, String par9Str, String par10Str
   ) {
      if (this.playerEntityList.isEmpty()) {
         ...
         for (int var15 = 0; var15 < this.playerEntityList.size(); var15++) {
            ...
            if (par9Str != null) {
               boolean var17 = par9Str.startsWith("!");
               if (var17) {
                  par9Str = par9Str.substring(1);
               }

               if (var17 == par9Str.equalsIgnoreCase(var16.am())) {
                  continue;
               }
            }

            if (par10Str != null) {
               boolean var17x = par10Str.startsWith("!");
               if (var17x) {
                  par10Str = par10Str.substring(1);
               }

               ScorePlayerTeam var18 = var16.cs();
               String var19 = var18 == null ? "" : var18.func_96661_b();
               if (var17x == par10Str.equalsIgnoreCase(var19)) {
                  continue;
               }
            }
            ...
            if (this.func_96457_a(var16, par8Map)
               && (par5 == EnumGameType.NOT_SET.getID() || par5 == var16.theItemInWorldManager.getGameType().getID())
               && (par6 <= 0 || var16.experienceLevel >= par6)
               && var16.experienceLevel <= par7) {
   ...
   private boolean func_96457_a(EntityPlayer par1EntityPlayer, Map par2Map) {
      if (par2Map != null && par2Map.size() != 0) {
         for (Entry var4 : par2Map.entrySet()) {
   ...
   public void updateTimeAndWeatherForPlayer(EntityPlayerMP par1EntityPlayerMP, WorldServer par2WorldServer) {
      par1EntityPlayerMP.playerNetServerHandler.sendPacketToPlayer(new Packet4UpdateTime(par2WorldServer.H(), par2WorldServer.I()));
      if (par2WorldServer.P()) {
   ...
   public String[] getAvailablePlayerDat() {
      return this.mcServer.worldServers[0].L().getSaveHandler().getAvailablePlayerDat();
   }
   ...
   public EntityPlayerMP respawnPlayer(EntityPlayerMP oldPlayer, int iDefaultDimension, boolean bPlayerLeavingTheEnd) {
      ...
      this.mcServer.worldServerForDimension(oldPlayer.dimension).f(oldPlayer);
      ...
      if (oldPlayer.hasRespawnCoordinates()) {
         if (!bPlayerLeavingTheEnd) {
            ...
            int iReturnValue = oldPlayer.getValidatedRespawnCoordinates(this.mcServer.worldServerForDimension(oldPlayer.spawnDimension), rawRespawnCoords);
            if (iReturnValue == 0) {
               verifiedRespawnCoords = rawRespawnCoords;
               iNewDimension = oldPlayer.spawnDimension;
            } else if (iReturnValue == 1) {
               spawnFailMessage = Block.beacon.a() + ".invalid";
            } else if (iReturnValue == 2) {
               spawnFailMessage = Block.beacon.a() + ".notPresent";
            } else if (iReturnValue == 3) {
               spawnFailMessage = Block.beacon.a() + ".tooFar";
               bRetainPreviousSpawn = true;
            } else if (iReturnValue == 4) {
               spawnFailMessage = Block.beacon.a() + ".obstructed";
               bRetainPreviousSpawn = true;
            } else {
               spawnFailMessage = Block.beacon.a() + ".unknown";
            }
         } else {
            bRetainPreviousSpawn = true;
         }
      ...
      EntityPlayerMP newPlayer = (EntityPlayerMP)EntityList.createEntityOfType(
         EntityPlayerMP.class, this.mcServer, this.mcServer.worldServerForDimension(iNewDimension), oldPlayer.username, worldManager
      );
      ...
      WorldServer newWorldServer = this.mcServer.worldServerForDimension(oldPlayer.dimension);
      this.func_72381_a(newPlayer, oldPlayer, newWorldServer);
      if (verifiedRespawnCoords != null) {
         newPlayer.b(verifiedRespawnCoords.posX + 0.5F, verifiedRespawnCoords.posY + 0.1F, verifiedRespawnCoords.posZ + 0.5F, 0.0F, 0.0F);
         bRetainPreviousSpawn = true;
         ...
         HardcoreSpawnUtils.handleHardcoreSpawn(this.mcServer, oldPlayer, newPlayer);
      }

      if (bRetainPreviousSpawn) {
         newPlayer.setSpawnChunk(oldPlayer.ck(), oldPlayer.cl(), oldPlayer.spawnDimension);
      }
      ...
      while (!newWorldServer.a(newPlayer, newPlayer.boundingBox).isEmpty()) {
         newPlayer.b(newPlayer.posX, newPlayer.posY + 1.0, newPlayer.posZ);
      }
      ...
      newPlayer.playerNetServerHandler
         .sendPacketToPlayer(
            new Packet9Respawn(
               newPlayer.dimension,
               (byte)newPlayer.worldObj.difficultySetting,
               newPlayer.worldObj.getWorldInfo().getTerrainType(),
               newPlayer.worldObj.getHeight(),
               newPlayer.theItemInWorldManager.getGameType()
            )
         );
      newPlayer.playerNetServerHandler.sendPacketToPlayer(new HardcoreSpawnPacket(newPlayer.timeOfLastSpawnAssignment));
      verifiedRespawnCoords = newWorldServer.J();
      newPlayer.playerNetServerHandler
         .sendPacketToPlayer(new Packet6SpawnPosition(verifiedRespawnCoords.posX, verifiedRespawnCoords.posY, verifiedRespawnCoords.posZ));
      ...
      newWorldServer.d(newPlayer);
      this.playerEntityList.add(newPlayer);
      newPlayer.addSelfToInternalCraftingInventory();
      newPlayer.setEntityHealth(newPlayer.aX());
   ...
   private void flagChunksAroundTeleportingEntityForCheckForUnload(WorldServer world, Entity entity) {
      ...
      int iChunkRange = 9;
      world.addChunkRangeToCheckForUnloadList(iChunkX - iChunkRange, iChunkZ - iChunkRange, iChunkX + iChunkRange, iChunkZ + iChunkRange);
   }
}
