...
public class ItemInWorldManager {
   ...
   public void updateBlockRemoving() {
      ...
      if (this.receivedFinishDiggingPacket) {
         ...
         if (var2 == 0) {
            ...
            float var4 = var3.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, this.posX, this.posY, this.posZ) * (var1 + 1);
         ...
         int var1 = this.theWorld.getBlockId(this.partiallyDestroyedBlockX, this.partiallyDestroyedBlockY, this.partiallyDestroyedBlockZ);
         Block var6 = Block.blocksList[var1];
         if (var6 == null) {
            this.theWorld
               .destroyBlockInWorldPartially(
                  this.thisPlayerMP.entityId, this.partiallyDestroyedBlockX, this.partiallyDestroyedBlockY, this.partiallyDestroyedBlockZ, -1
               );
            this.durabilityRemainingOnBlock = -1;
            this.isDestroyingBlock = false;
         } else {
            int var7 = this.curblockDamage - this.initialDamage;
            float var4x = var6.getPlayerRelativeBlockHardness(
                  this.thisPlayerMP, this.thisPlayerMP.worldObj, this.partiallyDestroyedBlockX, this.partiallyDestroyedBlockY, this.partiallyDestroyedBlockZ
               )
               * (var7 + 1);
            int var5x = (int)(var4x * 10.0F);
            if (var5x != this.durabilityRemainingOnBlock) {
               this.theWorld
                  .destroyBlockInWorldPartially(
                     this.thisPlayerMP.entityId, this.partiallyDestroyedBlockX, this.partiallyDestroyedBlockY, this.partiallyDestroyedBlockZ, var5x
                  );
               this.durabilityRemainingOnBlock = var5x;
            }
         }
      }
   }
   ...
   public void onBlockClicked(int par1, int par2, int par3, int par4) {
      if (!this.gameType.isAdventure() || this.thisPlayerMP.e(par1, par2, par3)) {
         if (this.isCreative()) {
   ...
   public void uncheckedTryHarvestBlock(int par1, int par2, int par3, int iFromFacing) {
      if (par1 == this.partiallyDestroyedBlockX && par2 == this.partiallyDestroyedBlockY && par3 == this.partiallyDestroyedBlockZ) {
         int var4x = this.curblockDamage - this.initialDamage;
         int var5 = this.theWorld.getBlockId(par1, par2, par3);
         if (var5 != 0) {
            Block var6 = Block.blocksList[var5];
            float var7 = var6.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, par1, par2, par3) * (var4x + 1);
            if (var7 >= 0.7F) {
               this.isDestroyingBlock = false;
               ...
               this.harvestingFromFacing = iFromFacing;
            }
         }
      }
   }
   ...
   public boolean activateBlockOrUseItem(
      EntityPlayer par1EntityPlayer, World par2World, ItemStack par3ItemStack, int par4, int par5, int par6, int par7, float par8, float par9, float par10
   ) {
      if (!par1EntityPlayer.ag() || par1EntityPlayer.getHeldItem() == null) {
   ...
   public boolean tryHarvestBlock(int i, int j, int k) {
      return this.tryHarvestBlock(i, j, k, 1);
   }

   public boolean tryHarvestBlock(int i, int j, int k, int iFromSide) {
      if (this.gameType.isAdventure() && !this.thisPlayerMP.e(i, j, k)) {
         return false;
      } else if (this.isCreative()) {
         int iBlockID = this.theWorld.getBlockId(i, j, k);
         int iBlockMetadata = this.theWorld.getBlockMetadata(i, j, k);
         this.theWorld.playAuxSFXAtEntity(this.thisPlayerMP, 2001, i, j, k, iBlockID + (this.theWorld.getBlockMetadata(i, j, k) << 12));
         boolean bBlockRemoved = this.removeBlock(i, j, k);
         WorldUtils.sendPacketToPlayer(this.thisPlayerMP.playerNetServerHandler, new Packet53BlockChange(i, j, k, this.theWorld));
         return bBlockRemoved;
      } else {
         return this.survivalTryHarvestBlock(i, j, k, iFromSide);
      }
   }
   ...
   public boolean survivalTryHarvestBlock(int i, int j, int k, int iFromSide) {
      ...
      if (block == null) {
         ...
         int iMetadata = this.theWorld.getBlockMetadata(i, j, k);
         this.theWorld.playAuxSFXAtEntity(this.thisPlayerMP, 2001, i, j, k, iBlockID + (iMetadata << 12));
         ItemStack currentStack = this.thisPlayerMP.cd();
         boolean bRemovingBlock = true;
         boolean bConvertingBlock = false;
         boolean bHarvestingBlock = this.thisPlayerMP.canHarvestBlock(Block.blocksList[iBlockID], i, j, k);
         if (!bHarvestingBlock) {
            bConvertingBlock = block.canConvertBlock(currentStack, this.theWorld, i, j, k);
            if (bConvertingBlock) {
               bRemovingBlock = !block.convertBlock(currentStack, this.theWorld, i, j, k, iFromSide);
            }
         }

         if (bRemovingBlock) {
            bRemovingBlock = this.removeBlock(i, j, k);
            if (bRemovingBlock && !bConvertingBlock) {
               if (bHarvestingBlock) {
                  Block.blocksList[iBlockID].harvestBlock(this.theWorld, this.thisPlayerMP, i, j, k, iMetadata);
               } else {
                  Block.blocksList[iBlockID].onBlockDestroyedWithImproperTool(this.theWorld, this.thisPlayerMP, i, j, k, iMetadata);
               }
            }
         }

         if (currentStack != null) {
            currentStack.onBlockDestroyed(this.theWorld, iBlockID, i, j, k, this.thisPlayerMP);
            if (currentStack.stackSize == 0) {
               this.thisPlayerMP.ce();
            }
         }
