...
public class MapData extends WorldSavedData {
      ...
      if (par2ItemStack.isOnItemFrame()) {
         if (this.isEntityLocationVisibleOnMap(par2ItemStack.getItemFrame())) {
            this.func_82567_a(
               ...
               "frame-" + par2ItemStack.getItemFrame().entityId,
               par2ItemStack.getItemFrame().xPosition,
               par2ItemStack.getItemFrame().zPosition,
               par2ItemStack.getItemFrame().hangingDirection * 90 + 180
            );
         }
      } else {
         ...
         if (!par1EntityPlayer.isDead
            && par1EntityPlayer.inventory.hasItemStack(par2ItemStack)
            && this.isEntityLocationVisibleOnMap(par1EntityPlayer)
            && par1EntityPlayer.dimension == this.dimension) {
            this.func_82567_a(
               0,
               par1EntityPlayer.worldObj,
   ...
   private void func_82567_a(int par1, World par2World, String par3Str, double par4, double par6, double par8) {
      ...
      float var11 = (float)(par4 - this.xCenter) / var10;
      float var12 = (float)(par6 - this.zCenter) / var10;
      ...
      if (var11 >= -var16 && var12 >= -var16 && var11 <= var16 && var12 <= var16) {
         ...
         var15 = 0;
         if (var11 <= -var16) {
            var13 = (byte)(var16 * 2 + 2.5);
         }

         if (var12 <= -var16) {
            var14 = (byte)(var16 * 2 + 2.5);
         }
   ...
   public boolean isEntityLocationVisibleOnMap(Entity entity) {
      ...
      return entity.dimension == this.dimension ? this.isLocationVisibleOnMap(entity.worldObj, i, j, k) : false;
   }

   public boolean isLocationVisibleOnMap(World world, int i, int j, int k) {
      int iMapScale = 1 << this.scale;
      float fRelativeI = (float)((double)i - this.xCenter) / iMapScale;
      float fRelativeK = (float)((double)k - this.zCenter) / iMapScale;
      if (Math.abs(fRelativeI) > 64.0F || Math.abs(fRelativeK) > 64.0F) {
         return false;
      } else if (!world.canBlockSeeTheSky(i, j, k) && world.getTopSolidOrLiquidBlock(i, k) > j) {
         return false;
      } else {
