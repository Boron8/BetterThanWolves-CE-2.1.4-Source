...
public class TreeUtils {
   public static boolean generateTaiga2(World world, Random random, int i, int j, int k) {
      ...
      if (j >= 1) {
         ...
         if (j + l + 1 <= 128) {
            ...
            if (canSaplingGrowOnBlock(world, i, j - 1, k)) {
               world.getClass();
               if (j < 128 - l - 1) {
                  ...
                  for (int l3 = 0; l3 <= j1; l3++) {
                     ...
                     for (int l4 = i - i2; l4 <= i + i2; l4++) {
                        int j5 = l4 - i;

                        for (int k5 = k - i2; k5 <= k + i2; k5++) {
                           int l5 = k5 - k;
                           if ((Math.abs(j5) != i2 || Math.abs(l5) != i2 || i2 <= 0) && !Block.opaqueCubeLookup[world.getBlockId(l4, j4, k5)]) {
   ...
   public static boolean generateForest(World world, Random random, int i, int j, int k) {
   ...
   public static boolean generateTrees(
      World world, Random random, int iBase, int jBase, int kBase, int iBaseHeight, int iWoodMetadata, int iLeafMetadata, boolean bGenerateVines
   ) {
      ...
      if (jBase >= 1 && jBase + iTreeHeight + 1 <= 256) {
            ...
            for (int l = iBase - byte0; l <= iBase + byte0 && flag; l++) {
               for (int j1 = kBase - byte0; j1 <= kBase + byte0 && flag; j1++) {
                  if (j >= 0 && j < 256) {
                     ...
                     if (!world.isAirBlock(l, j, j1)
                        && j2 != Block.leaves.blockID
         ...
         if (!flag) {
            ...
            if (canSaplingGrowOnBlock(world, iBase, jBase - 1, kBase) && jBase < 256 - iTreeHeight - 1) {
               ...
               for (int k1 = jBase - byte1 + iTreeHeight; k1 <= jBase + iTreeHeight; k1++) {
                  ...
                  for (int l3 = iBase - j3; l3 <= iBase + j3; l3++) {
                     ...
                     for (int l4 = kBase - j3; l4 <= kBase + j3; l4++) {
                        int i5 = l4 - kBase;
                        if ((Math.abs(j4) != j3 || Math.abs(i5) != j3 || random.nextInt(2) != 0 && k2 != 0)
                           && !Block.opaqueCubeLookup[world.getBlockId(l3, k1, l4)]) {
   ...
   private static void placeVine(World par1World, int par2, int par3, int par4, int par5) {
   ...
   public static boolean generateTrees(World world, Random random, int i, int j, int k) {
      return generateTrees(world, random, i, j, k, 4, 0, 0, false);
   }

   public static boolean canSaplingGrowOnBlock(World world, int i, int j, int k) {
