...
public class StoneBrickSlabBlock extends BlockHalfSlab {
   ...
   public String getFullSlabName(int iMetadata) {
      return super.a() + ".smoothStoneBrick";
   }

   @Override
   protected ItemStack createStackedBlock(int iMetadata) {
      return new ItemStack(BTWBlocks.stoneBrickSlab.blockID, 2, this.getStrata(iMetadata));
   }

   @Override
   public boolean dropComponentItemsOnBadBreak(World world, int i, int j, int k, int iMetadata, float fChanceOfDrop) {
      int numItems = this.isDoubleSlab ? 2 : 1;
      this.dropItemsIndividually(world, i, j, k, BTWBlocks.looseStoneBrickSlab.blockID, numItems, this.getStrata(iMetadata) << 2, fChanceOfDrop);
      return true;
   }
